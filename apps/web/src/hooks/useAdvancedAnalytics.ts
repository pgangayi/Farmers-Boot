...
export function useAdvancedAnalytics(farmId: number) {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`/functions/v1/analytics-metrics?${params}`, {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`/functions/v1/analytics-insights?${params}`, {
        headers: getAuthHeaders(),
      });
  ...
  const { data: predictiveModels, isLoading: predictiveLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/functions/v1/analytics-predictive?farm_id=${farmId}`, {
        headers: getAuthHeaders(),
      });
  ...
  const { data: performanceScore, isLoading: performanceLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`/functions/v1/analytics-performance?${params}`, {
        headers: getAuthHeaders(),
      });
  ...
  const { data: benchmarks, isLoading: benchmarksLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`/functions/v1/analytics-benchmarks?${params}`, {
        headers: getAuthHeaders(),
      });
  ...
  const generateReport = useCallback(
    async (reportConfig: {
      ...
      const response = await fetch('/functions/v1/analytics-report', {
        method: 'POST',
        headers: {
...
export function useRealTimeAnalytics(farmId: number) {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(
      `${window.location.protocol === 'https:' ? 'wss:' : 'ws:'}//${window.location.host}/realtime/v1/websocket?stream=analytics&farm_id=${farmId}`
    );
