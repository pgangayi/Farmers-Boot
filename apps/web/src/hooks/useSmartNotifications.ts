...
const NOTIFICATION_RULES: NotificationRule[] = [
  {
    ...
    condition: (data: unknown) =>
      (data as InventoryNotificationData).quantity <=
        (data as InventoryNotificationData).minStock &&
      (data as InventoryNotificationData).quantity > 0,
    action: (context: unknown) => {
      const ctx = context as InventoryNotificationData;
      return {
        type: 'inventory',
        title: 'Low Stock Warning',
        message: `${ctx.name} is running low (${ctx.quantity} remaining)`,
        priority: 'medium',
        category: 'inventory',
        data: ctx,
        actionable: true,
        actionUrl: `/inventory/${ctx.id}`,
        actionLabel: 'Restock',
      };
    },
    enabled: true,
  ...
  },
  {
    ...
    type: 'task',
    condition: (data: unknown) =>
      new Date((data as TaskNotificationData).dueDate) < new Date() &&
      (data as TaskNotificationData).status !== 'completed',
    action: (context: unknown) => {
      const ctx = context as TaskNotificationData;
      return {
        type: 'task',
        title: 'Overdue Task',
        message: `Task "${ctx.title}" is overdue`,
        priority: 'high',
        category: 'task',
        data: ctx,
        actionable: true,
        actionUrl: `/tasks/${ctx.id}`,
  ...
  },
  {
    ...
    type: 'animal',
    condition: (data: unknown) => (data as AnimalNotificationData).healthStatus === 'sick',
    action: (context: unknown) => {
      const ctx = context as AnimalNotificationData;
      return {
        type: 'animal',
        title: 'Health Alert',
        message: `${ctx.name} needs medical attention`,
        priority: 'high',
        category: 'health',
        data: ctx,
        actionable: true,
        actionUrl: `/animals/${ctx.id}`,
  ...
  },
  {
    ...
    type: 'weather',
    condition: (data: unknown) =>
      (data as WeatherNotificationData).temperature > 40 ||
      (data as WeatherNotificationData).temperature < 0,
    action: (context: unknown) => {
      const ctx = context as WeatherNotificationData;
      return {
        type: 'weather',
        title: 'Temperature Alert',
        message: `Temperature extreme: ${ctx.temperature}°C`,
        priority: 'urgent',
        category: 'weather',
        data: ctx,
        actionable: false,
  ...
  },
  {
    ...
    type: 'crop',
    condition: (data: unknown) =>
      (data as CropNotificationData).daysToHarvest <= 7 &&
      (data as CropNotificationData).daysToHarvest >= 0,
    action: (context: unknown) => {
      const ctx = context as CropNotificationData;
      return {
        type: 'crop',
        title: 'Harvest Ready',
        message: `${ctx.name} will be ready for harvest in ${ctx.daysToHarvest} days`,
        priority: 'medium',
        category: 'crop',
        data: ctx,
        actionable: true,
        actionUrl: `/crops/${ctx.id}`,
  ...
  const checkForNotifications = useCallback(async () => {
    ...
    try {
      const [inventoryRes, tasksRes, animalsRes, weatherRes, cropsRes] = await Promise.allSettled([
        fetch('/rest/v1/inventory'),
        fetch('/rest/v1/tasks'),
        fetch('/rest/v1/animals'),
        fetch('/rest/v1/weather'),
        fetch('/rest/v1/crops'),
      ]);
