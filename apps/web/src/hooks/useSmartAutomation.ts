...
// Constants for API endpoints and query keys to avoid duplication
const ENDPOINT_AUTOMATION_RULES = '/functions/v1/automation-rules';
const QUERY_KEY_AUTOMATION_RULES = 'automation-rules';
...
export function useSmartAutomation(farmId: number) {
  ...
  const {
    ...
    queryKey: [QUERY_KEY_AUTOMATION_RULES, farmId],
    queryFn: async () => {
      const response = await fetch(`${ENDPOINT_AUTOMATION_RULES}?farm_id=${farmId}`, {
  ...
  const { data: executions, isLoading: executionsLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/functions/v1/automation-executions?farm_id=${farmId}&limit=50`,
        {
          headers: getAuthHeaders(),
        }
      );
  ...
  const createRuleMutation = useMutation({
    mutationFn: async (
      ...
      const response = await fetch(ENDPOINT_AUTOMATION_RULES, {
        method: 'POST',
        headers: {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY_AUTOMATION_RULES, farmId] });
    },
  });
  ...
  const updateRuleMutation = useMutation({
    mutationFn: async ({ id, updates }: { id: string; updates: Partial<AutomationRule> }) => {
      const response = await fetch(`${ENDPOINT_AUTOMATION_RULES}?id=eq.${id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          ...getAuthHeaders(),
  ...
  const executeRuleMutation = useMutation({
    mutationFn: async (ruleId: string) => {
      const response = await fetch(`/functions/v1/automation-execute?ruleId=${ruleId}`, {
        method: 'POST',
        headers: getAuthHeaders(),
      });
