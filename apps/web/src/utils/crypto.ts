  ...
  static encode(data: string): string {
    ...
    try {
      ...
      for (let i = 0; i < obfuscated.length; i++) {
        ...
        const secretIndex = i % secretBytes.length;
        const dataIndex = i % dataBytes.length;

        const saltByte = salt[saltIndex] || 0;
        const secretByte = secretBytes[secretIndex] || 0;
        const dataByte = dataBytes[dataIndex] || 0;
  ...
  static decode(encodedData: string): string | null {
    ...
    try {
      ...
      for (let i = 0; i < obfuscated.length; i++) {
        const saltIndex = i % salt.length;
        const secretIndex = i % secretBytes.length;
        const obfuscatedIndex = i;

        const saltByte = salt[saltIndex] || 0;
        const secretByte = secretBytes[secretIndex] || 0;
        const obfuscatedByte = obfuscated[obfuscatedIndex] || 0;
